# Best Practices for Using Saas Control-M to Run a Pod to Completion in a Kubernetes-based Cluster

## Running Control-M Workloads in Kubernetes

This readme describes how to run Saas Control-M workloads in Kubernetes-based clusters, including OpenShift (from
RedHat) and public cloud platforms such as EKS (from Amazon), AKS (from Microsoft Azure), or GKE (from Google). The
information in this document focuses on the scenario of running a pod to completion with a Saas Control-M/Agent running
within the cluster.

The document demonstrates how to do this while leveraging Saas Control-M scheduling and monitoring capabilities.
...
a Linux environment with the September 2022 release of Saas Control-M.
...
There are several ways to run a Kubernetes (K8s) pod to completion using Saas Control-M.

This document describes a selected methodology for utilizing a Saas Control-M Application Integrator job type to run
your Saas Control-M workloads in Kubernetes.

The Control-M Application Integrator job starts a Kubernetes job entity and monitors its status until the job ends.

After the job ends, the pod’s output is captured and is presented in the Saas Control-M job output. The Kubernetes job
...
startup. This enables Saas Control-M to uniquely identify the Saas Control-M/Agent continuously and consistently.

The pod uses a persistent volume, so that job data and Saas Control-M/Agent state are kept during shutdown and startup.
...
| Dockerfile                   | A file that contains instructions for how to construct a Saas Control-M/Agent Docker image                                                                                                |
| startup.sh                   | A bash script that is used to start up the Saas Control-M/Agent                                                                                                                           |
...
| stateful_ha.yaml             | A file that contains StatefulSet definitions for running a Saas Control-M/Agent container, Kubernetes definitions for creation of the Persistent Volume Claim, and a linkage between them |
...
- For building and executing the Saas Control-M/Agent container and deploying the Application Integrator job type:
...
A typical process of deploying Saas Control-M/Agent on Kubernetes consists of the following steps:

- [Step 1: Create a Saas Control-M/Agent image](#step-1-create-a-saas-control-magent-image)
...
- [Step 5: Run Saas Control-M jobs](#step-5-run-control-m-jobs)
...
> support in deploying Saas Control-M in this manner, please request assistance from BMC Support.

### Step 1: Create a Saas Control-M/Agent image
...
- AAPI_CLI_URL - The full URL path of the .BIN installer file for Automation API CLI (for example,
  DR5V3.9.0.20.220_Linux-x86_64.BIN), as it appears in the Plugins section in Saas Control-M.
...
This produces an image that contains a Saas Control-M/Agent with kubectl and support for executing Control-M
...
2. Registers and configures the Agent in Saas Control-M.
...
1. In Saas Control-M, under the **Tools** menu, click **Application Integrator** to open Control-M Application
   Integrator.
...
This creates a new job type named KUBERNETES. This job type can now be used on any Agent associated with Saas
Control-M.
...
Saas Control-M/Agent is kept across shutdown and startup of the pod.
...
1. Edit the following parameters in the stateful_ha.yaml file:
    ...
    - AGENT_TOKEN_TAG - The token name of the agent, as it appears in Agent Token Management in Saas Control-M.
...
Saas Control-M/Agent to Saas Control-M.

To verify the connection between the Saas Control-M/Agent and Saas Control-M, run the following checks:

| Check                      | Command                                                                                                                                                                                                                                                                           | Expected Output                          |
|----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
| Saas Control-M/Agent side | ```kubectl exec -it ctm-statefulset-agent-0 -- bash -c "source .bash_profile && ag_ping"``` This command runs the ag_ping utility, which resides in the Saas Control-M/Agent image. The StatefulSet is named satefulset-agent, with a number suffix for the instance number (0). | Server is alive. Result: Success.        |
| Saas Control-M side       | Access Saas Control-M, navigate to the Configuration domain, and review the list of available agents.                                                                                                                                                                            | Agent : ctm-statefulset-agent-0 is alive |
...
### Step 5: Run Saas Control-M jobs

To run the Saas Control-M job, perform the following actions:
